...
function toPlacedNode([key, value]: [string, MenuItemInput], index: number): PlacedNode {
  ...
  return { key, node, parentKey: parent, order: order ?? ORDER_UNSPECIFIED, index }
}

/** The entry's link target: its key by default, or none when `href: false`. */
function resolveHref(key: string, href: string | false | undefined): string | undefined {
  if (href === false) return undefined
  return href ?? key
}

function appendChild(buckets: Map<string, PlacedNode[]>, parentKey: string, entry: PlacedNode): void {
