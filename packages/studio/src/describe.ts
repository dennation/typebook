import { createElement, type ComponentType } from 'react'
import type {
  Expand,
  DescribeConfig,
  DescribeResult,
  SingleStory,
  VariantsStory,
  MatrixStory,
  StoryRenderFn,
  AllOfConfig,
  ValuesConfig,
  GenerateConfig,
  VariantConfig,
} from './types.js'

export function describe<
  Props extends Record<string, any>,
  IncludedProps extends keyof Props = keyof Props,
  D extends Partial<Props> = Partial<Props>
>(
  component: ComponentType<Props>,
  config?: DescribeConfig<Props, IncludedProps> & { defaults?: D },
): DescribeResult<Expand<Pick<Props, IncludedProps>>, keyof D & IncludedProps> {
  const defaults: Record<string, unknown> = config?.defaults ?? {}
  const name = config?.name
  const path = config?.path
  const wrapper = config?.wrapper

  const defaultRender: StoryRenderFn = (props) => createElement(component, props)

  const wrapRender = (renderFn: StoryRenderFn): StoryRenderFn => {
    if (!wrapper) return renderFn
    return (props) => wrapper(() => renderFn(props) as any)
  }

  const result: DescribeResult<Expand<Pick<Props, IncludedProps>>, keyof D & IncludedProps> = {
    __type: 'define',
    component,
    name,
    path,
    defaults,

    // Story creation methods
    single(config?: { props?: any; render?: any; isolate?: boolean; name?: string; path?: string }): SingleStory {
      return {
        __type: 'story',
        kind: 'single',
        component,
        defaults,
        props: config?.props ? { ...config.props } : undefined,
        render: wrapRender(config?.render ?? defaultRender),
        isolate: config?.isolate,
        name: config?.name,
        path: config?.path,
      }
    },

    variants(config: {
      items: VariantConfig
      props?: any
      columns?: number
      isolate?: boolean
      name?: string
      path?: string
    }): VariantsStory {
      return {
        __type: 'story',
        kind: 'variants',
        component,
        defaults,
        items: config.items,
        props: config.props ? { ...config.props } : undefined,
        columns: config.columns,
        render: wrapRender(defaultRender),
        isolate: config.isolate,
        name: config.name,
        path: config.path,
      }
    },

    matrix(config: {
      x: VariantConfig
      y: VariantConfig[]
      props?: any
      isolate?: boolean
      name?: string
      path?: string
    }): MatrixStory {
      return {
        __type: 'story',
        kind: 'matrix',
        component,
        defaults,
        x: config.x,
        y: config.y,
        props: config.props ? { ...config.props } : undefined,
        render: wrapRender(defaultRender),
        isolate: config.isolate,
        name: config.name,
        path: config.path,
      }
    },

    // Variant config helpers
    allOf(prop: any): AllOfConfig {
      return {
        __type: 'allOf',
        prop: String(prop),
      }
    },

    values(prop: any, values: any[]): ValuesConfig {
      return {
        __type: 'values',
        prop: String(prop),
        values,
      }
    },

    generate(prop: any, fn: () => any, count: number): GenerateConfig {
      return {
        __type: 'generate',
        prop: String(prop),
        fn,
        count,
      }
    },
  }

  return result
}
