...
export function ComponentPreview({
  ...
  return (
    <div className="st:border st:border-border st:rounded-lg st:overflow-hidden st:mb-6">
      {/* Preview — full width */}
          ...
          <table className="st:w-full st:text-xs">
            <thead>
              <tr className="st:border-b st:border-border">
                <th className="st:text-left st:py-1.5 st:px-3 st:font-semibold st:text-text-muted">Prop</th>
                <th className="st:text-left st:py-1.5 st:px-3 st:font-semibold st:text-text-muted">Type</th>
                <th className="st:text-left st:py-1.5 st:px-3 st:font-semibold st:text-text-muted">Control</th>
              </tr>
            </thead>
            <tbody>
              {props.map((prop) => (
                <tr key={prop.name} className="st:border-b st:border-border last:st:border-b-0">
                  <td className="st:py-1.5 st:px-3 st:font-mono st:text-accent st:whitespace-nowrap">
                    {prop.name}
                    {!prop.optional && <span className="st:text-red-400 st:ml-0.5">*</span>}
                  </td>
                  <td className="st:py-1.5 st:px-3 st:font-mono st:text-text-muted st:max-w-[200px] st:truncate" title={formatType(prop)}>
                    {formatType(prop)}
                  </td>
                  <td className="st:py-1.5 st:px-3">
                    {isControllable(prop) ? (
                      <PropControl
                        prop={prop}
