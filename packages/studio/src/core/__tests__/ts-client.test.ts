...
function extractProps(storyFile: string): Promise<PropInfo[] | null> {
	return client.getComponentProps(storyFile)
}
...
describe('basic types', () => {
	...
	test('optional string literal union → literal kind + optional', () => {
		...
		expect(variant.type).toEqual({ kind: 'literal', values: ['solid', 'outline', 'ghost'] })
...
describe('inheritance: extends', () => {
	...
	beforeAll(async () => {
		const result = await extractProps('stories/WithInheritanceExtends.stories.tsx')
		expect(result).not.toBeNull()
		props = result!
	})
	...
	test('includes own props (variant, disabled)', () => {
		const variant = findProp(props, 'variant')!
		expect(variant.type).toEqual({ kind: 'literal', values: ['primary', 'secondary'] })
...
describe('inheritance: intersection', () => {
	...
	beforeAll(async () => {
		const result = await extractProps('stories/WithInheritanceIntersection.stories.tsx')
		expect(result).not.toBeNull()
		props = result!
	})
	...
	test('includes intersection-added props', () => {
		...
		expect(target.type).toEqual({ kind: 'literal', values: ['_blank', '_self'] })
...
describe('nullable types', () => {
	...
	test('literal | undefined → literal (undefined filtered)', () => {
		...
		expect(status.type).toEqual({ kind: 'literal', values: ['active', 'inactive'] })
	})
...
describe('conditional and advanced types', () => {
	...
	beforeAll(async () => {
		const result = await extractProps('stories/WithConditionalTypes.stories.tsx')
		expect(result).not.toBeNull()
		props = result!
	})
