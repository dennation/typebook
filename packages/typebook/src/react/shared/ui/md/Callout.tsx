...
export function Callout({ type = "info", title, children }: CalloutProps) {
	...
	return (
		<div
			className="flex gap-3.25 p-[15px_17px] rounded-(--radius-token) text-[14.5px] leading-[1.6] border border-[color-mix(in_oklch,var(--c)_35%,var(--border))] bg-[color-mix(in_oklch,var(--c)_8%,var(--bg))] [&_.inline-code]:bg-bg"
			style={{ "--c": CALLOUT_C[type] } as CSSProperties}
		>
			<span className="shrink-0 mt-px text-(--c) inline-flex">
				<Ic size={19} />
			</span>
