 ...
 */
function collectSnippetNames(body: Array<Record<string, unknown>>): Set<string> {
  const names = new Set<string>()
  for (const node of body) {
    if (node.type !== 'ImportDeclaration') continue
    if ((node.source as { value?: string } | undefined)?.value !== NPM_REACT_PACKAGE_NAME) continue
...
function isSnippetTag(opening: Record<string, unknown>, snippetLocalNames: Set<string>): boolean {
  const name = opening.name as { type?: string; name?: string } | undefined
  return name?.type === 'JSXIdentifier' && !!name.name && snippetLocalNames.has(name.name)
}
...
function nameAttribute(opening: Record<string, unknown>): string | null {
  const attributes = (opening.attributes as Array<Record<string, unknown>>) ?? []
  for (const attr of attributes) {
    if (attr.type !== 'JSXAttribute') continue
    if ((attr.name as { name?: string } | undefined)?.name !== 'name') continue
    return staticStringValue(attr.value as Record<string, unknown> | null | undefined)
  }
  return null
}

/** Resolve `name="x"` (Literal) and `name={'x'}` (JSXExpressionContainer → Literal). */
function staticStringValue(value: Record<string, unknown> | null | undefined): string | null {
...
function extractChildrenSource(
  ...
  const closing = element.closingElement as Record<string, unknown> | null | undefined
  if (!closing) return ''
