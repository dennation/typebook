...
- a51b309: The core now only scans components and dispatches commands — plugins bring their own commands and do their own work. `typebook <command>` runs whatever the configured plugins registered; `dev` / `build` in the config name which of them a bundler repeats.
