/**
 * Test: deeper expansion strategies to resolve Size/Variant aliases.
 */
...
async function run() {
  ...
  // Strategy 1: ExpandDeep — recursively expand each property
  const strategy1 = originalContent + `
type __ExpandDeep<T> = T extends (...args: infer A) => infer R ? (...args: A) => R : T extends object ? { [K in keyof T]: __ExpandDeep<T[K]> } : T
type __Props1 = __ExpandDeep<React.ComponentProps<typeof ComposedButton>>
`

  notify('textDocument/didChange', {
    textDocument: { uri, version: 2 },
    contentChanges: [{ text: strategy1 }],
  })
  await new Promise(r => setTimeout(r, 2000))

  const lines1 = strategy1.split('\n')
  console.log('--- Strategy 1: ExpandDeep ---')
  console.log(await hover(uri, lines1.length - 2, 6))

  // Strategy 2: Simplify — just flatten to primitives
  const strategy2 = originalContent + `
type __Simplify<T> = { [K in keyof T]: T[K] } & {}
type __Props2 = __Simplify<React.ComponentProps<typeof ComposedButton>>
`

  notify('textDocument/didChange', {
    textDocument: { uri, version: 3 },
    contentChanges: [{ text: strategy2 }],
  })
  await new Promise(r => setTimeout(r, 2000))

  const lines2 = strategy2.split('\n')
  console.log('\n--- Strategy 2: Simplify (& {}) ---')
  console.log(await hover(uri, lines2.length - 2, 6))
