...
packages/studio/
  ...
  src/
    ...
    react/
      ...
      context.ts              — StudioMetaContext (Component → ComponentMeta map) + StudioWrapperContext (global storyWrapper) + InspectContext (inspect modal state, InspectedData) + CodeThemeContext (shiki themes)
      components/
        index.ts              — Barrel export
        Studio.tsx            — <Studio /> component (per-story sidebar tree, theme, single-story rendering, inspect modal)
        ...
        InspectModal.tsx      — Fullscreen inspect modal: live component preview, Props table, Code preview, Action log
        inspectUtils.tsx      — Shared sub-components for inspect: SectionHeader, PropsTable, LogEntryRow, FilterDropdown, formatters
...
- **ComponentPreview** — unified component rendering used by both `VariantCard` and `Playground`. Handles: `wrapActionProps` (action logging + stub generation), `storyWrapper` application, `IsolateWrapper` (conditional iframe), `ErrorBoundary`, and inspect button. Reads `InspectContext` to show/highlight the inspect button and pass full `InspectedData` on click.
- **Inspect Modal** — fullscreen modal (`InspectModal.tsx`) opened by clicking the inspect button on any preview. Layout: live component preview (left), Props table + Code preview (right, resizable vertical split), Action log (bottom). Component is re-rendered live inside the modal with action logging. Props are read-only. Action log filtered to the inspected previewId only. Closes via Escape, X button, or clicking the same inspect button again. Sub-components extracted to `inspectUtils.tsx`.
- **InspectContext** — provides `{ inspectedPreviewId, onInspect }` to the component tree. `onInspect` receives full `InspectedData` (previewId, component, props, render, isolate, trackActions) — no shared refs needed. `ComponentPreview` reads `inspectedPreviewId` to highlight the active inspect button. `Studio.tsx` stores `InspectedData | null` and renders `InspectModal` conditionally.
