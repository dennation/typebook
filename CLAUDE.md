...
- **Opaque per-item `meta`, generic.** Items carry consumer metadata (badge, deprecated flag, counters — anything) via a generic `M` that threads through the whole chain (`MenuItem<M>` → `Menu<M>` → `MenuItemInput<Parent, M>` → `MenuItemProps<M>`/`MenuComponents<M>`). `M` defaults to **`never`** — a menu with no meta type has no usable `meta` (it's `undefined`). Give a shape via `defineMenu<MyMeta>(…)` (the generic is the **first** type param so `parent`-checking key inference still works) and: `meta` is **optional when authoring** (`MenuItemInput`) but **required on the output node** (`MenuItem`) — `defineMenu` guarantees its presence, defaulting a missing one to `{}`, so the consumer's `Item` (typed `MenuItemProps<MyMeta>`) reads `item.meta.badge` with **no optional chaining**. `M` should therefore be an all-optional-fields shape (the badge/flags use-case). The renderer never reads `meta`; the resolver stays meta-agnostic (internals run at `unknown`, the result is cast to `Menu<M>`). `menuFromRouteTree` reads it from `RouteMenuMeta<M>.meta`.
