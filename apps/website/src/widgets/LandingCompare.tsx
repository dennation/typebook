...
const P = (t: string) => <span className="text-[12px] text-fg-subtle italic">{t}</span>

const ROWS: { f: string; s: string; v: ReactNode[] }[] = [
	{ f: 'Write in Markdown / MDX', s: 'No proprietary authoring format', v: [Y, Y, Y, Y] },
	{ f: 'Filesystem-driven sidebar', s: 'Folders map to navigation', v: [Y, Y, P('partial'), N] },
	{ f: 'Zero-config search', s: 'Built-in, no external service', v: [Y, P('plugin'), P('plugin'), Y] },
	{ f: 'Token-level theming', s: 'Restyle from one CSS file', v: [Y, P('limited'), Y, P('limited')] },
	{ f: 'Self-host, no lock-in', s: 'Your repo, your CDN', v: [Y, Y, Y, N] },
	{ f: 'Free & open source', s: 'MIT licensed', v: [Y, Y, Y, N] },
	{ f: 'Ships components', s: 'Callouts, tabs, steps, cards', v: [Y, P('basic'), P('basic'), Y] },
]
...
export function LandingCompare() {
	return (
		<section className={cx(SECTION_PAD, 'bg-bg-secondary border-t border-b border-border')} id="compare">
			<div className={CONTAINER}>
				...
				<div className="overflow-x-auto reveal">
					<table className="w-full border-separate border-spacing-0 min-w-[720px] bg-bg border border-border rounded-[14px] overflow-hidden shadow-md">
						<thead>
							<tr>
								<th className={cx(CELL, FEAT_CELL, 'bg-bg-secondary align-bottom')}>&nbsp;</th>
								{COLS.map((c, i) => (
									<th
										key={i}
										className={cx(
									...
									>
										<span className={cx('text-[15px] font-[650] tracking-[-0.01em] block', c.own ? 'text-accent' : 'text-fg')}>
											{c.name}
										</span>
										<span className="block text-[11px] font-[450] text-fg-subtle mt-[2px]">{c.tag}</span>
									</th>
								))}
							</tr>
						</thead>
								...
								return (
									<tr key={i}>
										<td className={cx(CELL, FEAT_CELL, last && 'border-b-0')}>
											{r.f}
											<small className="block font-normal text-[12.5px] text-fg-subtle mt-[2px]">{r.s}</small>
										</td>
										{r.v.map((v, k) => (
											<td
												key={k}
												className={cx(
													...
													'text-center',
													COLS[k].own
														? cx(OWN_CELL, last && 'border-b border-b-accent-soft-border')
														: last && 'border-b-0',
												)}
											>
												{v}
