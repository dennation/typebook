...
const FEATURES: Feature[] = [
	{
		...
		list: [
			...
			<>Works offline and in static exports</>,
		],
	},
	{
		...
		list: [
			...
			<>No route config, no manual nav arrays</>,
			<>
	...
	},
	{
		...
		list: [
			...
			<>Highlighted code with copy & line marks</>,
			<>
				...
				<div className="flex flex-col gap-[96px]">
					{FEATURES.map((f, i) => (
						<div
							...
							key={i}
						>
							<div className={cx('max-w-[460px] max-[860px]:max-w-none', f.flip && 'min-[861px]:order-2')}>
								...
								<h3 className="text-[26px] font-[640] tracking-[-0.025em] leading-[1.15] m-0 mb-[12px]">{f.title}</h3>
								<p className="text-[16px] leading-[1.65] text-fg-muted m-0 mb-[18px] [text-wrap:pretty]">{f.body}</p>
								<ul className="flex flex-col gap-[14px] m-0 p-0 list-none [&_b]:text-fg [&_b]:font-semibold">
									{f.list.map((li, k) => (
										<li className="flex items-start gap-[11px] text-[14.5px] text-fg-muted leading-[1.45]" key={k}>
							...
							<div className={cx('min-w-0', f.flip && 'min-[861px]:order-1')}>{f.demo}</div>
						</div>
					))}
				</div>
			</div>
