...
StackBlitz playground: Vite + React + typebook, 2-3 компонента (Button, Input, Card), все три вида историй.
...
`typebook-meta.gen.ts` коммитится в git — полная история PropInfo уже хранится в репозитории. Не нужно отдельное хранилище снэпшотов.

**Механизм:**
1. `git tag --sort=version:sort` — список всех версий
2. Для каждого тега: `git show <tag>:typebook-meta.gen.json` — PropInfo на тот момент
3. Diff последовательных пар тегов по компонентам
4. Склейка в полную историю

**Предусловие:** генерировать `typebook-meta.gen.json` параллельно с `.ts` — чистый JSON, тривиально парсится без AST.

**CLI:**
```bash
npx @dennation/typebook changelog                        # полная история всех компонентов
npx @dennation/typebook changelog --component Button     # один компонент
npx @dennation/typebook changelog --since v1.0.0         # с определённой версии
```
...
Новый entry point `@dennation/typebook/testing`:
```ts
import { composeStories } from '@dennation/typebook/testing'
...
Поэтапный план:
1. **Visual Snapshot Testing** — `npx @dennation/typebook test` → скриншоты всех вариантов, diff с предыдущими. CI-интеграция. Playwright для скриншотов.
